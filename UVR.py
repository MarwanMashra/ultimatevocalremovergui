...
class ModelData:
    def __init__(
        ...
        self.is_deverb_vocals = (
            root.is_deverb_vocals_var.get() if os.path.isfile(DEVERBER_MODEL_PATH) else False
        )
        self.deverb_vocal_opt = DEVERB_MAPPER[root.deverb_vocal_opt_var.get()]
        self.is_denoise_model = (
            True
            if root.denoise_option_var.get() == DENOISE_M and os.path.isfile(DENOISER_MODEL_PATH)
            else False
        )
        ...
        self.mdx_batch_size = (
            1 if root.mdx_batch_size_var.get() == DEF_OPT else int(root.mdx_batch_size_var.get())
        )
        ...
        if self.process_method == VR_ARCH_TYPE:
            self.is_secondary_model_activated = (
                root.vr_is_secondary_model_activate_var.get() if not is_secondary_model else False
            )
        ...
        if self.process_method == MDX_ARCH_TYPE:
            self.is_secondary_model_activated = (
                root.mdx_is_secondary_model_activate_var.get() if not is_secondary_model else False
            )
        ...
        if self.process_method == DEMUCS_ARCH_TYPE:
            self.is_secondary_model_activated = (
                root.demucs_is_secondary_model_activate_var.get() if not is_secondary_model else False
            )
        ...
        if is_secondary_activated_and_status:
            if is_valid_ensemble or self.is_4_stem_ensemble or is_multi_stem_ensemble_demucs:
                ...
                primary_stem = (
                    self.ensemble_primary_stem if self.is_ensemble_mode and is_demucs else self.primary_stem
                )
...
class Ensembler:
    ...
    def ensemble_outputs(self, audio_file_base, export_path, stem, is_4_stem=False, is_inst_mix=False):
        ...
        if is_4_stem:
            ...
            if is_inst_mix:
                ...
                stem_tag = (
                    self.ensemble_primary_stem if stem == PRIMARY_STEM else self.ensemble_secondary_stem
                )
    ...
    def get_files_to_ensemble(self, folder="", prefix="", suffix=""):
        ...
        return [
            os.path.join(folder, i) for i in os.listdir(folder) if i.startswith(prefix) and i.endswith(suffix)
        ]

    def combine_audio(self, audio_inputs, audio_file_base):
        save_format_ = lambda save_path: save_format(
...
class ListboxBatchFrame(tk.Frame):
    ...
    def delete_selected(self):
        ...
        if selected:
            basename = self.listbox.get(selected[0]).split(": ", 1)[
                1
...
class ComboBoxEditableMenu(ttk.Combobox):
    ...
    def var_validation(self, is_focus_only=False, is_start_up=False):
        ...
        if re.fullmatch(self.pattern, self.textvariable.get()) is None:
            if not is_start_up and not self.textvariable.get() in (
                OPT_SEPARATOR,
                USER_INPUT,
            ):
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def __init__(self):
        ...
        self.geometry(
            "{width}x{height}+{xpad}+{ypad}".format(
        ...
        self.vr_secondary_model_vars = {
            "voc_inst_secondary_model": self.vr_voc_inst_secondary_model_var,
        ...
        self.demucs_secondary_model_vars = {
            "voc_inst_secondary_model": self.demucs_voc_inst_secondary_model_var,
        ...
        self.mdx_secondary_model_vars = {
            "voc_inst_secondary_model": self.mdx_voc_inst_secondary_model_var,
    ...
    # Menu Functions
    def main_window_LABEL_SET(self, master, text):
        return ttk.Label(
            master=master,
            text=text,
            background=BG_COLOR,
    ...
    def main_window_LABEL_SUB_SET(self, master, text_var):
        return ttk.Label(
            master=master,
    ...
    def menu_title_LABEL_SET(self, frame, text, width=35):
        return ttk.Label(
            master=frame,
            text=text,
            font=(SEC_FONT_NAME, f"{FONT_SIZE_5}", "underline"),
            justify="center",
            foreground="#13849f",
            width=width,
            anchor=tk.CENTER,
    ...
    def menu_sub_LABEL_SET(self, frame, text, font_size=FONT_SIZE_2):
        return ttk.Label(
            master=frame,
            text=text,
            font=(MAIN_FONT_NAME, f"{font_size}"),
            foreground=FG_COLOR,
            anchor=tk.CENTER,
        )

    def menu_FRAME_SET(self, frame, thickness=20):
        return tk.Frame(
            frame,
            highlightbackground=BG_COLOR,
            highlightcolor=BG_COLOR,
            highlightthicknes=thickness,
        )

    def check_is_menu_settings_open(self):
        self.menu_settings() if not self.is_menu_settings_open else None

    def spacer_label(self, frame):
        return tk.Label(
            frame,
            text="",
    ...
    def ensemble_listbox_get_all_selected_models(self):
        return [self.ensemble_listbox_Option.get(i) for i in self.ensemble_listbox_Option.curselection()]

    def ensemble_listbox_select_from_indexs(self, indexes):
        return [self.ensemble_listbox_Option.selection_set(i) for i in indexes]

    def ensemble_listbox_clear_and_insert_new(self, model_ensemble_updated):
        return (
            self.ensemble_listbox_Option.delete(0, "end"),
            [self.ensemble_listbox_Option.insert(tk.END, models) for models in model_ensemble_updated],
        )

    def ensemble_listbox_get_indexes_for_files(self, updated, selected):
        return [updated.index(model) for model in selected if model in updated]
    ...
    def assemble_model_data(
        ...
        if arch_type == KARAOKEE_CHECK:
            ...
            model_data: List[ModelData] = [
                ModelData(model_name, is_dry_check=is_dry_check)
                for model_name in self.default_change_model_list
            ]
        ...
        if arch_type == ENSEMBLE_MODE:
            model_data: List[ModelData] = [
                ModelData(model_name) for model_name in self.ensemble_listbox_get_all_selected_models()
            ]
    ...
    def fill_main_frame(self):
        ...
        self.conversion_Button = ttk.Button(
            master=self,
            textvariable=self.conversion_Button_Text_var,
            command=self.process_initialize,
        )
        self.conversion_Button.place(
            x=X_CONVERSION_BUTTON_1080P,
            y=BUTTON_Y_1080P,
            width=WIDTH_CONVERSION_BUTTON_1080P,
            height=HEIGHT_GENERIC_BUTTON_1080P,
            relx=0,
            rely=0,
            relwidth=1,
            relheight=0,
        ...
        self.conversion_Button_enable = lambda: (
            self.conversion_Button_Text_var.set(START_PROCESSING),
            self.conversion_Button.configure(state=tk.NORMAL),
        )
        self.conversion_Button_disable = lambda message: (
            self.conversion_Button_Text_var.set(message),
        ...
        self.stop_Button.place(
            x=X_STOP_BUTTON_1080P,
            y=BUTTON_Y_1080P,
            width=HEIGHT_GENERIC_BUTTON_1080P,
            height=HEIGHT_GENERIC_BUTTON_1080P,
            relx=1,
            rely=0,
            relwidth=0,
        ...
        self.progressbar.place(
            x=X_PROGRESSBAR_1080P,
            y=Y_OFFSET_PROGRESS_BAR_1080P,
            width=WIDTH_PROGRESSBAR_1080P,
            height=HEIGHT_PROGRESSBAR_1080P,
            relx=0,
            rely=0,
            relwidth=1,
            relheight=0,
        ...
        self.console_Frame = tk.Frame(
            master=self,
            highlightbackground="#101012",
            highlightcolor="#101012",
            highlightthicknes=2,
        )
        self.console_Frame.place(
            x=15,
            y=self.IMAGE_HEIGHT
            + self.FILEPATHS_HEIGHT
            + self.OPTIONS_HEIGHT
            + self.CONVERSIONBUTTON_HEIGHT
            + self.PADDING
            + 5 * 3,
            width=-30,
            height=self.COMMAND_HEIGHT + 7,
            relx=0,
            rely=0,
            relwidth=1,
            relheight=0,
    ...
    def fill_filePaths_Frame(self):
        ...
        self.filePaths_Frame.place(
            x=FILEPATHS_FRAME_X,
            y=FILEPATHS_FRAME_Y,
            width=FILEPATHS_FRAME_WIDTH,
            height=self.FILEPATHS_HEIGHT,
            relx=0,
            rely=0,
            relwidth=1,
            relheight=0,
        ...
        self.filePaths_musicFile_Button = ttk.Button(
            master=self.filePaths_Frame,
            text=SELECT_INPUT_TEXT,
            command=self.input_select_filedialog,
        )
        self.filePaths_musicFile_Button.place(
            x=MUSICFILE_BUTTON_X,
            y=MUSICFILE_BUTTON_Y,
            width=MUSICFILE_BUTTON_WIDTH,
            height=MUSICFILE_BUTTON_HEIGHT,
            relx=0,
            rely=0,
            relwidth=0.3,
            relheight=0.5,
        ...
        self.filePaths_musicFile_Entry.place(
            x=MUSICFILE_ENTRY_X,
            y=MUSICFILE_BUTTON_Y,
            width=MUSICFILE_ENTRY_WIDTH,
            height=MUSICFILE_ENTRY_HEIGHT,
            relx=0.3,
            rely=0,
            relwidth=0.7,
            relheight=0.5,
        )
        self.filePaths_musicFile_Open = ttk.Button(
            master=self.filePaths_Frame,
            image=self.efile_img,
            command=lambda: OPEN_FILE_func(os.path.dirname(self.inputPaths[0]))
            if self.inputPaths and os.path.isdir(os.path.dirname(self.inputPaths[0]))
            else self.error_dialoge(INVALID_INPUT),
        )
        self.filePaths_musicFile_Open.place(
            x=OPEN_BUTTON_X,
            y=MUSICFILE_BUTTON_Y,
        ...
        self.filePaths_saveTo_Button = ttk.Button(
            master=self.filePaths_Frame,
            text=SELECT_OUTPUT_TEXT,
            command=self.export_select_filedialog,
        )
        self.filePaths_saveTo_Button.place(
            x=SAVETO_BUTTON_X,
            y=SAVETO_BUTTON_Y,
            width=SAVETO_BUTTON_WIDTH,
            height=SAVETO_BUTTON_HEIGHT,
            relx=0,
            rely=0.5,
            relwidth=0.3,
            relheight=0.5,
        ...
        self.filePaths_saveTo_Entry.place(
            x=SAVETO_ENTRY_X,
            y=SAVETO_BUTTON_Y,
            width=SAVETO_ENTRY_WIDTH,
            height=SAVETO_ENTRY_HEIGHT,
            relx=0.3,
            rely=0.5,
            relwidth=0.7,
            relheight=0.5,
        )
        self.filePaths_saveTo_Open = ttk.Button(
            master=self.filePaths_Frame,
            image=self.efile_img,
            command=lambda: OPEN_FILE_func(Path(self.export_path_var.get()))
            if os.path.isdir(self.export_path_var.get())
            else self.error_dialoge(INVALID_EXPORT),
        )
        self.filePaths_saveTo_Open.place(
            x=OPEN_BUTTON_X,
            y=SAVETO_BUTTON_Y,
            width=OPEN_BUTTON_WIDTH,
            height=SAVETO_ENTRY_HEIGHT,
    ...
    def fill_options_Frame(self):
        ...
        self.wav_button.place(
            x=RADIOBUTTON_X_WAV,
            y=RADIOBUTTON_Y,
            width=RADIOBUTTON_WIDTH,
            height=RADIOBUTTON_HEIGHT,
            relx=0,
            rely=0 / self.COL2_ROWS,
            relwidth=1 / 3,
            relheight=1 / self.COL2_ROWS,
        )
        ...
        self.chosen_process_method_Label.place(
            ...
            height=LABEL_HEIGHT,
            relx=0,
            rely=2 / self.COL1_ROWS,
            relwidth=1 / 3,
            relheight=1 / self.COL1_ROWS,
        )
        self.chosen_process_method_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.chosen_process_method_var,
            values=PROCESS_METHODS,
            command=lambda e: self.selection_action_process_method(
                self.chosen_process_method_var.get(),
                from_widget=True,
        ...
        self.save_current_settings_Label_place = lambda: self.save_current_settings_Label.place(
            ...
            height=LABEL_HEIGHT,
            relx=2 / 3,
            rely=6 / self.COL1_ROWS,
            relwidth=1 / 3,
            relheight=1 / self.COL1_ROWS,
        )
        self.save_current_settings_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.save_current_settings_var,
        ...
        self.mdx_net_model_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.mdx_net_model_var,
        ...
        self.overlap_mdx_Option = ComboBoxEditableMenu(
            ...
            textvariable=self.overlap_mdx_var,
            pattern=REG_OVERLAP,
            default=MDX_OVERLAP,
        )
        ...
        self.overlap_mdx23_Option = ComboBoxEditableMenu(
            ...
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.overlap_mdx23_var,
        ...
        self.mdx_segment_size_Option = ComboBoxEditableMenu(
            ...
            values=MDX_SEGMENTS,
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.mdx_segment_size_var,
            pattern=REG_MDX_SEG,
            default="256",
        )  #
        ...
        self.vr_model_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.vr_model_var,
        ...
        self.demucs_model_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.demucs_model_var,
        ...
        # Stem A
        self.is_primary_stem_only_Demucs_Option = ttk.Checkbutton(
            master=self.options_Frame,
            textvariable=self.is_primary_stem_only_Demucs_Text_var,
            variable=self.is_primary_stem_only_Demucs_var,
            command=lambda: self.is_primary_stem_only_Demucs_Option_toggle(),
        )
        self.is_primary_stem_only_Demucs_Option_place = lambda: self.is_primary_stem_only_Demucs_Option.place(
            ...
            relx=1 / 3,
            rely=6 / self.COL2_ROWS,
            relwidth=1 / 3,
            relheight=1 / self.COL2_ROWS,
        )
        self.is_primary_stem_only_Demucs_Option_toggle = (
            lambda: self.is_secondary_stem_only_Demucs_var.set(False)
            if self.is_primary_stem_only_Demucs_var.get()
            else self.is_secondary_stem_only_Demucs_Option.configure(state=tk.NORMAL)
        )
        self.help_hints(self.is_primary_stem_only_Demucs_Option, text=SAVE_STEM_ONLY_HELP)

        # Stem B
        self.is_secondary_stem_only_Demucs_Option = ttk.Checkbutton(
            master=self.options_Frame,
            textvariable=self.is_secondary_stem_only_Demucs_Text_var,
            variable=self.is_secondary_stem_only_Demucs_var,
            command=lambda: self.is_secondary_stem_only_Demucs_Option_toggle(),
        )
        self.is_secondary_stem_only_Demucs_Option_place = (
            lambda: self.is_secondary_stem_only_Demucs_Option.place(
                x=CHECK_BOX_X,
                y=CHECK_BOX_Y,
                width=CHECK_BOX_WIDTH,
                height=CHECK_BOX_HEIGHT,
                relx=1 / 3,
                rely=7 / self.COL2_ROWS,
                relwidth=1 / 3,
                relheight=1 / self.COL2_ROWS,
            )
        )
        self.is_secondary_stem_only_Demucs_Option_toggle = (
            lambda: self.is_primary_stem_only_Demucs_var.set(False)
            if self.is_secondary_stem_only_Demucs_var.get()
            else self.is_primary_stem_only_Demucs_Option.configure(state=tk.NORMAL)
        )
        self.is_stem_only_Demucs_Options_Enable = lambda: (
        ...
        self.chosen_ensemble_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.chosen_ensemble_var,
        ...
        self.ensemble_main_stem_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.ensemble_main_stem_var,
            values=ENSEMBLE_MAIN_STEM,
            command=lambda e: self.selection_action_ensemble_stems(self.ensemble_main_stem_var.get()),
        )
        ...
        self.ensemble_type_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.ensemble_type_var,
            values=ENSEMBLE_TYPE,
        )
        ...
        self.ensemble_listbox_Option = tk.Listbox(
        ...
        self.ensemble_listbox_Option_place = lambda: (
            self.ensemble_listbox_Frame.place(
            ...
            self.ensemble_listbox_scroll.place(
        ...
        self.chosen_audio_tool_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.chosen_audio_tool_var,
            values=AUDIO_TOOL_OPTIONS,
            command=lambda e: self.update_main_widget_states(),
        )
        ...
        self.choose_algorithm_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.choose_algorithm_var,
        ...
        self.time_stretch_rate_Option = ComboBoxEditableMenu(
            self.options_Frame,
            values=TIME_PITCH,
            textvariable=self.time_stretch_rate_var,
            pattern=REG_TIME,
            default=TIME_PITCH,
        )  #
        ...
        self.pitch_rate_Option = ComboBoxEditableMenu(
            self.options_Frame,
            values=TIME_PITCH,
            textvariable=self.pitch_rate_var,
            pattern=REG_PITCH,
            default=TIME_PITCH,
        )  #
        ...
        self.is_time_correction_Option = ttk.Checkbutton(
            ...
            variable=self.is_time_correction_var,
        )
        ...
        self.is_wav_ensemble_Option = ttk.Checkbutton(
            ...
            variable=self.is_wav_ensemble_var,
        )
        ...
        self.fileOne_Entry = ttk.Entry(
            master=self.options_Frame,
            textvariable=self.fileOneEntry_var,
            font=self.font_entry,
            state=tk.DISABLED,
        )
        ...
        self.fileOne_Open = ttk.Button(
            master=self.options_Frame,
            image=self.efile_img,
            command=lambda: OPEN_FILE_func(os.path.dirname(self.fileOneEntry_Full_var.get())),
        )
        self.fileOne_Open_place = (
            lambda: self.fileOne_Open.place(
        ...
        self.fileTwo_Entry = ttk.Entry(
            master=self.options_Frame,
            textvariable=self.fileTwoEntry_var,
            font=self.font_entry,
            state=tk.DISABLED,
        )
        ...
        self.fileTwo_Open = ttk.Button(
            master=self.options_Frame,
            image=self.efile_img,
            command=lambda: OPEN_FILE_func(os.path.dirname(self.fileTwoEntry_Full_var.get())),
        )
        ...
        self.intro_analysis_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.intro_analysis_var,
        ...
        self.db_analysis_Option = ComboBoxMenu(
            self.options_Frame,
            textvariable=self.db_analysis_var,
        ...
        self.is_gpu_conversion_Option = ttk.Checkbutton(
            ...
            variable=self.is_gpu_conversion_var,
        )
        ...
        self.is_gpu_conversion_Disable = lambda: (
            self.is_gpu_conversion_Option.configure(state=tk.DISABLED),
            self.is_gpu_conversion_var.set(False),
        )
        ...
        # Vocal Only
        self.is_primary_stem_only_Option = ttk.Checkbutton(
            master=self.options_Frame,
            textvariable=self.is_primary_stem_only_Text_var,
            variable=self.is_primary_stem_only_var,
            command=lambda: self.is_primary_stem_only_Option_toggle(),
        )
        ...
        self.is_primary_stem_only_Option_toggle = (
            lambda: self.is_secondary_stem_only_var.set(False)
            if self.is_primary_stem_only_var.get()
            else self.is_secondary_stem_only_Option.configure(state=tk.NORMAL)
        )
        self.help_hints(self.is_primary_stem_only_Option, text=SAVE_STEM_ONLY_HELP)

        # Instrumental Only
        self.is_secondary_stem_only_Option = ttk.Checkbutton(
            master=self.options_Frame,
            textvariable=self.is_secondary_stem_only_Text_var,
            variable=self.is_secondary_stem_only_var,
            command=lambda: self.is_secondary_stem_only_Option_toggle(),
        )
        ...
        self.is_secondary_stem_only_Option_toggle = (
            lambda: self.is_primary_stem_only_var.set(False)
            if self.is_secondary_stem_only_var.get()
            else self.is_primary_stem_only_Option.configure(state=tk.NORMAL)
        )
        self.is_stem_only_Options_Enable = lambda: (
        ...
        self.model_sample_mode_Option = ttk.Checkbutton(
            master=self.options_Frame,
            textvariable=self.model_sample_mode_duration_checkbox_var,
            variable=self.model_sample_mode_var,
    ...
    def focus_out_widgets(self, all_widgets, frame):
        for option in all_widgets:
            if not type(option) is ComboBoxEditableMenu:
                option.bind(
                    "<Button-1>",
                    lambda e: (option.focus(), self.combo_box_selection_clear(frame)),
                )
    ...
    def bind_widgets(self):
        ...
        all_widgets = (
            self.options_Frame.winfo_children() + self.filePaths_Frame.winfo_children() + other_items
        )
        ...
        self.ensemble_listbox_Option.bind(
            "<<ListboxSelect>>",
            lambda e: self.chosen_ensemble_var.set(CHOOSE_ENSEMBLE_OPTION),
        )
        self.options_Frame.bind(
            right_click_button,
            lambda e: (
                self.right_click_menu_popup(e, main_menu=True),
                self.options_Frame.focus(),
            ),
        )
        self.filePaths_musicFile_Entry.bind(
            right_click_button,
            lambda e: (
                self.input_right_click_menu(e),
                self.filePaths_musicFile_Entry.focus(),
            ),
        )
        self.filePaths_musicFile_Entry.bind(
            "<Button-1>",
            lambda e: (
                self.check_is_menu_open(INPUTS_MENU),
                self.filePaths_musicFile_Entry.focus(),
            ),
        )
        ...
        self.fileOne_Entry.bind(
            right_click_button,
            lambda e: self.input_dual_right_click_menu(e, is_primary=True),
        )
        self.fileTwo_Entry.bind(
            right_click_button,
            lambda e: self.input_dual_right_click_menu(e, is_primary=False),
        )
        if not is_macos:
    ...
    def show_file_dialog(self, text="Select Audio files", dialoge_type=None):
        ...
        if dialoge_type == MULTIPLE_FILE:
            filenames = filedialog.askopenfilenames(parent=parent_win, title=text)
        elif dialoge_type == MAIN_MULTIPLE_FILE:
            filenames = filedialog.askopenfilenames(
                parent=parent_win, title=text, initialfile="", initialdir=self.lastDir
            )
        elif dialoge_type == SINGLE_FILE:
            filenames = filedialog.askopenfilename(parent=parent_win, title=text)
    ...
    def select_audiofile(self, path=None, is_primary=True):
        ...
        vars = {
            True: (
                self.fileOneEntry_Full_var,
                self.fileOneEntry_var,
                self.fileTwoEntry_Full_var,
                self.fileTwoEntry_var,
            ),
            False: (
                self.fileTwoEntry_Full_var,
                self.fileTwoEntry_var,
    ...
    def restart(self):
        ...
        confirm = messagebox.askyesno(
            parent=root, title=CONFIRM_RESTART_TEXT[0], message=CONFIRM_RESTART_TEXT[1]
        )
    ...
    def message_box(self, message):
        ...
        confirm = messagebox.askyesno(title=message[0], message=message[1], parent=root)
    ...
    def error_dialoge(self, message):
        ...
        messagebox.showerror(master=self, title=message[0], message=message[1], parent=root)
    ...
    def model_list(
        ...
        def matches_stem(model: ModelData):
            ...
            return (
                primary_match or mdx_stem_match
                if is_no_demucs
                else primary_match or primary_stem in model.mdx_model_stems
            )
        ...
        for model in stem_check:
            if is_multi_stem:
                ...
                result.append(model.model_and_process_tag)
            elif matches_stem(model) or (
                not is_no_demucs and primary_stem.lower() in model.demucs_source_list
            ):
                if is_check_vocal_split:
                    model_name = (
                        None if model.is_karaoke or not model.vocal_split_model else model.model_basename
                    )
                else:
    ...
    def input_dual_right_click_menu(self, event, is_primary: bool):
        ...
        right_click_menu = tk.Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
        right_click_menu.add_command(
            label=CHOOSE_INPUT_TEXT,
            command=lambda: self.select_audiofile(is_primary=is_primary),
        )
        if input_path and os.path.isdir(os.path.dirname(input_path)):
            right_click_menu.add_command(
                label=OPEN_INPUT_DIR_TEXT,
                command=lambda: OPEN_FILE_func(os.path.dirname(input_path)),
            )
    ...
    def cached_source_model_list_check(self, model_list: List[ModelData]):
        ...
        secondary_model_names = lambda process_method: [
            ...
            if model.is_secondary_model_activated and model.process_method == process_method
            else None
        ...
        self.demucs_pre_proc_model_name = [
            model.pre_proc_model.model_basename if model.pre_proc_model else None for model in model_list
        ]  # list(dict.fromkeys())
            ...
            if os.path.isfile(i):
                try:
                    librosa.load(i, duration=3, mono=False, sr=44100) if not type(
                        sample_path
                    ) is str else self.create_sample(i, sample_path)
                    is_good = True
                except Exception as e:
    ...
    def create_sample(self, audio_file, sample_path=SAMPLE_CLIP_PATH):
        ...
        sample = librosa.load(audio_file, offset=offset_cut, duration=clip_duration, mono=False, sr=44100)[
            0
        ].T
    ...
    def right_click_select_settings_sub(self, parent_menu, process_method):
        ...
        for settings_options in settings_options:
            ...
            saved_settings_sub_menu.add_command(
                label=settings_options,
                command=lambda o=settings_options: self.selection_action_saved_settings(
    ...
    def right_click_menu_popup(self, event, text_box=False, main_menu=False):
        ...
        def add_advanced_settings_options(menu, settings_mapper, var_mapper):
            ...
            if current_method in settings_mapper and (
                ...
                or (current_method == DEMUCS_ARCH_TYPE and self.is_demucs_pre_proc_model_activate_var.get())
            ):
        ...
        if not main_menu:
            ...
            if (
                ...
                and self.chosen_audio_tool_var.get() == ALIGN_INPUTS
            ):
                right_click_menu.add_command(
                    label="Advanced Align Tool Settings",
                    command=lambda: self.check_is_menu_open(ALIGNMENT_TOOL),
                )
            else:
            ...
            if not self.is_menu_settings_open:
                right_click_menu.add_command(
                    ...
                    command=lambda: self.menu_settings(select_tab_2=True),
                )

            help_hints_label = "Enable" if not self.help_hints_var.get() else "Disable"
            right_click_menu.add_command(
                label=f"{help_hints_label} Help Hints",
                command=lambda: self.help_hints_var.set(not self.help_hints_var.get()),
            )

            if self.error_log_var.get():
                right_click_menu.add_command(
                    label="Error Log",
                    command=lambda: self.check_is_menu_open(ERROR_OPTION),
                )
    ...
    def vocal_splitter_Button_opt(self, top_window, frame, pady, width=15):
        vocal_splitter_Button = ttk.Button(
            frame,
            text=VOCAL_SPLITTER_OPTIONS_TEXT,
            command=lambda: self.pop_up_set_vocal_splitter(top_window),
            width=width,
        )  #
    ...
    def menu_placement(
        ...
        def right_click_menu(event):
            ...
            if is_help_hints:
                right_click_menu.add_command(
                    label=f"{help_hints_label} Help Hints",
                    command=lambda: self.help_hints_var.set(help_hints_bool),
                )
    ...
    def menu_tab_control(self, toplevel, ai_network_vars, is_demucs=False, is_mdxnet=False):
        ...
        if is_demucs or is_mdxnet:
            ...
            tabControl.add(
                tab3,
                text=PREPROCESS_MODEL_CHOOSE_TEXT if is_demucs else MDX23C_ONLY_OPTIONS_TEXT,
            )
        ...
        def on_tab_selected(event):
            ...
            if event.widget.tab("current", option="text") == "Secondary Model" and not self.tab2_loaded:
                ...
                event.widget.tab("current", option="text") == PREPROCESS_MODEL_CHOOSE_TEXT
                and not self.tab3_loaded
            ):
                tab = tab3
            ...
            if load_screen:
                ...
                loading_label = ttk.Label(
                    ...
                    font=Font(family=MAIN_FONT_NAME, size=14),
                )
    ...
    def menu_view_inputs(self):
        ...
        def selected_files(is_remove=False):
            if not self.thread_check(self.active_processing_thread):
                ...
                [
                    input_files_listbox_Option.delete(input_files_listbox_Option.get(0, tk.END).index(i))
        ...
        def box_size():
            ...
            input_files_listbox_Option.config(
                width=230, height=25
            ) if is_widen_box_var.get() else input_files_listbox_Option.config(width=110, height=17)
        ...
        def right_click_menu(event):
            right_click_menu = tk.Menu(self, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=0)
            right_click_menu.add_command(
                label="Remove Selected Items Only",
                command=lambda: selected_files(is_remove=True),
            )
            right_click_menu.add_command(
                label="Keep Selected Items Only",
                command=lambda: selected_files(is_remove=False),
            )
            right_click_menu.add_command(
                label="Clear All Input(s)",
                command=lambda: input_options(is_select_inputs=False),
            )
            right_click_menu.add_separator()
            right_click_menu_sub = tk.Menu(
                right_click_menu, font=(MAIN_FONT_NAME, FONT_SIZE_1), tearoff=False
            )
            right_click_menu.add_command(
                ...
                right_click_menu_sub.add_command(
                    ...
                    command=lambda: (
                        input_files_listbox_Option.bind(
                            "<Double-Button>",
                            lambda e: pop_open_file_path(is_play_file=True),
                        ),
        ...
        tk.Label(
            ...
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_1}"),
            foreground=FG_COLOR,
        ).grid(row=1, column=0, padx=0, pady=MENU_PADDING_1)
    ...
    def menu_batch_dual(self):
        ...
        def clear_all(event, lb):
            ...
            if selected:
                ...
                right_click_menu.add_command(
                    label="Open File",
                    command=lambda: open_selected_path(lb, is_play_file=True),
                )
        ...
        left_frame = ListboxBatchFrame(
            menu_view_inputs_Frame,
            self.file_one_sub_var.get().title(),
            move_entry,
            self.right_img,
            self.img_mapper,
        )
        ...
        right_frame = ListboxBatchFrame(
            menu_view_inputs_Frame,
            self.file_two_sub_var.get().title(),
    ...
    def check_dual_paths(self, is_fill_menu=False):
        if self.DualBatch_inputPaths:
            ...
            if first_paths_len == 1:
                ...
                file1_base_text = (
                    f"{os.path.basename(first_paths[0])}, +{first_paths_len} file(s){BATCH_MODE_DUAL}"
                )
                file2_base_text = (
                    f"{os.path.basename(first_paths[1])}, +{first_paths_len} file(s){BATCH_MODE_DUAL}"
                )
            ...
            if is_fill_menu:
                ...
                if (
                    file_one
                    and file_two
                    and BATCH_MODE_DUAL not in file_one
                    and BATCH_MODE_DUAL not in file_two
                ):
        ...
        try:
            if cuda_available:
                self.cuda_device_list = [
                    f"{torch.cuda.get_device_properties(i).name}:{i}"
    ...
    def menu_settings(self, select_tab_2=False, select_tab_3=False):  # **
        ...
        select_Option = ComboBoxMenu(
            ...
            textvariable=self.main_menu_var,
            values=OPTION_LIST,
        ...
        select_Option.update_dropdown_size(
            ...
            "menuchoose",
            command=lambda e: (
                self.check_is_menu_open(self.main_menu_var.get()),
                close_window(),
            ),
        )
        ...
        open_app_dir_Button = ttk.Button(
            ...
            command=lambda: OPEN_FILE_func(BASE_PATH),
        ...
        reset_all_app_settings_Button = ttk.Button(
            ...
            command=lambda: self.load_to_default_confirm(),
            width=SETTINGS_BUT_WIDTH,
        ...
        self.deletion_list_fill(
            delete_your_settings_Option,
            option_var,
            SETTINGS_CACHE_DIR,
            SELECT_SAVED_SETTING,
            menu_name="deletesetting",
        )
        ...
        self.app_update_button = ttk.Button(
            settings_menu_main_Frame,
            textvariable=self.app_update_button_Text_var,
            width=SETTINGS_BUT_WIDTH - 2,
            command=lambda: self.pop_up_update_confirmation(),
        )
        self.app_update_button.grid(pady=MENU_PADDING_1)

        self.app_update_status_Label = tk.Label(
            settings_menu_main_Frame,
            textvariable=self.app_update_status_Text_var,
            padx=3,
            pady=3,
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_4}"),
        ...
        wav_type_set_Option = ComboBoxMenu(
            ...
            textvariable=self.wav_type_set_var,
            values=WAV_TYPE,
            width=HELP_HINT_CHECKBOX_WIDTH,
        )
        ...
        mp3_bit_set_Option = ComboBoxMenu(
            settings_menu_format_Frame,
            textvariable=self.mp3_bit_set_var,
        ...
        is_testing_audio_Option = ttk.Checkbutton(
            ...
            variable=self.is_testing_audio_var,
        )
        ...
        is_add_model_name_Option = ttk.Checkbutton(
            ...
            width=GEN_SETTINGS_WIDTH,
            variable=self.is_add_model_name_var,
        )
        ...
        is_accept_any_input_Option = ttk.Checkbutton(
            ...
            width=GEN_SETTINGS_WIDTH,
            variable=self.is_accept_any_input_var,
        )
        ...
        is_task_complete_Option = ttk.Checkbutton(
            ...
            width=GEN_SETTINGS_WIDTH,
            variable=self.is_task_complete_var,
        )
        ...
        is_normalization_Option = ttk.Checkbutton(
            ...
            width=GEN_SETTINGS_WIDTH,
            variable=self.is_normalization_var,
        )
        ...
        change_model_default_Button = ttk.Button(
            ...
            command=lambda: self.pop_up_change_model_defaults(settings_menu),
            width=SETTINGS_BUT_WIDTH - 2,
        )  #
        ...
        if not is_macos and self.is_gpu_available:
            gpu_list_options = (
                lambda: self.loop_gpu_list(device_set_Option, "gpudevice", self.cuda_device_list)
            )  # self.opencl_list if is_opencl_only or self.is_use_opencl_var.get() else self.cuda_device_list)
            ...
            device_set_Option = ComboBoxMenu(
                settings_menu_format_Frame,
                textvariable=self.device_set_var,
        ...
        tk.Label(
            settings_menu_format_Frame,
            textvariable=self.model_sample_mode_duration_label_var,
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_1}"),
            foreground=FG_COLOR,
        ).grid(pady=2)
        model_sample_mode_duration_Option = ttk.Scale(
            settings_menu_format_Frame,
            variable=self.model_sample_mode_duration_var,
            from_=5,
            to=120,
        ...
        self.model_download_mdx_Option = ComboBoxMenu(
            settings_menu_download_center_Frame,
            textvariable=self.model_download_mdx_var,
            width=READ_ONLY_COMBO_WIDTH,
        )
        self.model_download_mdx_Option.grid(pady=MENU_PADDING_1)

        self.model_download_demucs_Button = ttk.Radiobutton(
            settings_menu_download_center_Frame,
            text="Demucs",
            width=8,
            variable=self.select_download_var,
        ...
        self.model_download_demucs_Option = ComboBoxMenu(
            settings_menu_download_center_Frame,
            textvariable=self.model_download_demucs_var,
            width=READ_ONLY_COMBO_WIDTH,
        )
        ...
        self.download_Button = ttk.Button(
            ...
            command=lambda: self.download_item(),
        )  # , command=download_model)
        self.download_Button.grid(pady=MENU_PADDING_1)

        self.download_progress_info_Label = tk.Label(
            settings_menu_download_center_Frame,
            textvariable=self.download_progress_info_var,
        ...
        self.download_progress_percent_Label = tk.Label(
            settings_menu_download_center_Frame,
            textvariable=self.download_progress_percent_var,
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_2}"),
            wraplength=350,
            foreground=FG_COLOR,
        )
        ...
        self.stop_download_Button = ttk.Button(
            settings_menu_download_center_Frame,
            textvariable=self.download_stop_var,
            width=15,
            command=lambda: self.download_post_action(DOWNLOAD_STOPPED),
        )
        self.stop_download_Button.grid(pady=MENU_PADDING_1)
        self.stop_download_Button_DISABLE = lambda: (
            self.download_stop_var.set(""),
            self.stop_download_Button.configure(state=tk.DISABLED),
        )
        self.stop_download_Button_ENABLE = lambda: (
            self.download_stop_var.set(STOP_DOWNLOAD_TEXT),
            self.stop_download_Button.configure(state=tk.NORMAL),
        )

        self.refresh_list_Button = ttk.Button(
            settings_menu_download_center_Frame,
            text=REFRESH_LIST_TEXT,
            command=lambda: self.online_data_refresh(refresh_list_Button=True),
        ...
        self.download_key_Button = ttk.Button(
            settings_menu_download_center_Frame,
            image=self.key_img,
            command=lambda: self.pop_up_user_code_input(),
        )
        ...
        self.manual_download_Button = ttk.Button(
            ...
            command=self.menu_manual_downloads,
        )
        self.manual_download_Button.grid(pady=MENU_PADDING_1)

        self.download_center_Buttons = (
            self.model_download_vr_Button,
    ...
    def menu_advanced_vr_options(self):  # **
        ...
        self.menu_advanced_vr_options_close_window = lambda: (
            self.is_open_menu_advanced_vr_options.set(False),
            vr_opt.destroy(),
        )
        vr_opt.protocol("WM_DELETE_WINDOW", self.menu_advanced_vr_options_close_window)

        toggle_post_process = (
            lambda: self.post_process_threshold_Option.configure(state=READ_ONLY)
            if self.is_post_process_var.get()
            else self.post_process_threshold_Option.configure(state=tk.DISABLED)
        )
        ...
        self.post_process_threshold_Option = ComboBoxEditableMenu(
            vr_opt_frame,
            values=POST_PROCESSES_THREASHOLD_VALUES,
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.post_process_threshold_var,
        ...
        self.is_post_process_Option = ttk.Checkbutton(
            ...
            width=VR_CHECKBOXS_WIDTH,
            variable=self.is_post_process_var,
        ...
        self.is_high_end_process_Option = ttk.Checkbutton(
            ...
            width=VR_CHECKBOXS_WIDTH,
            variable=self.is_high_end_process_var,
        )
        ...
        self.vr_clear_cache_Button = ttk.Button(
            ...
            command=lambda: self.clear_cache(VR_ARCH_TYPE),
            width=VR_BUT_WIDTH,
        )
        ...
        self.open_vr_model_dir_Button = ttk.Button(
            ...
            command=lambda: OPEN_FILE_func(VR_MODELS_DIR),
            width=VR_BUT_WIDTH,
        )
        self.open_vr_model_dir_Button.grid(pady=MENU_PADDING_1)

        self.vr_return_Button = ttk.Button(
            vr_opt_frame,
            text=BACK_TO_MAIN_MENU,
            command=lambda: (
                self.menu_advanced_vr_options_close_window(),
                self.check_is_menu_settings_open(),
            ),
        )
        ...
        self.vr_close_Button = ttk.Button(
            vr_opt_frame,
            text=CLOSE_WINDOW,
            command=lambda: self.menu_advanced_vr_options_close_window(),
        )
        ...
        self.menu_placement(
            ...
            is_help_hints=True,
            close_function=self.menu_advanced_vr_options_close_window,
    ...
    def menu_advanced_demucs_options(self):  # **
        ...
        self.menu_advanced_demucs_options_close_window = lambda: (
            self.is_open_menu_advanced_demucs_options.set(False),
            demuc_opt.destroy(),
        )
        ...
        self.shifts_Option = ComboBoxEditableMenu(
            demucs_frame,
            values=DEMUCS_SHIFTS,
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.shifts_var,
            pattern=REG_SHIFTS,
        ...
        pitch_shift_Option = ComboBoxEditableMenu(
            ...
            values=SEMITONE_SEL,
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.semitone_shift_var,
        ...
        self.is_split_mode_Option = ttk.Checkbutton(
            ...
            variable=self.is_split_mode_var,
        )
        ...
        self.is_demucs_combine_stems_Option = ttk.Checkbutton(
            ...
            width=DEMUCS_CHECKBOXS_WIDTH,
            variable=self.is_demucs_combine_stems_var,
        )
        ...
        is_invert_spec_Option = ttk.Checkbutton(
            ...
            width=DEMUCS_CHECKBOXS_WIDTH,
            variable=self.is_invert_spec_var,
        )
        ...
        self.open_demucs_model_dir_Button = ttk.Button(
            demucs_frame,
            text=OPEN_MODELS_FOLDER_TEXT,
            command=lambda: OPEN_FILE_func(DEMUCS_MODELS_DIR),
            width=VR_BUT_WIDTH,
        )
        self.open_demucs_model_dir_Button.grid(pady=MENU_PADDING_1)

        self.demucs_return_Button = ttk.Button(
            demucs_frame,
            text=BACK_TO_MAIN_MENU,
            command=lambda: (
                self.menu_advanced_demucs_options_close_window(),
        ...
        self.demucs_close_Button = ttk.Button(
            demucs_frame,
            text=CLOSE_WINDOW,
            command=lambda: self.menu_advanced_demucs_options_close_window(),
        )
        ...
        self.menu_placement(
            ...
            is_help_hints=True,
            close_function=self.menu_advanced_demucs_options_close_window,
            frame_list=frame_list,
        )
    ...
    def menu_advanced_mdx_options(self):  # **
        ...
        self.menu_advanced_mdx_options_close_window = lambda: (
            self.is_open_menu_advanced_mdx_options.set(False),
            mdx_net_opt.destroy(),
        )
        ...
        compensate_Option = ComboBoxEditableMenu(
            ...
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.compensate_var,
        ...
        denoise_option_Option = ComboBoxMenu(
            mdx_net_frame,
            textvariable=self.denoise_option_var,
        ...
        is_match_frequency_pitch_Option = ttk.Checkbutton(
            ...
            variable=self.is_match_frequency_pitch_var,
        )
        ...
        clear_mdx_cache_Button = ttk.Button(
            mdx_net_frame,
            text=CLEAR_AUTOSET_CACHE_TEXT,
            command=lambda: self.clear_cache(MDX_ARCH_TYPE),
            width=VR_BUT_WIDTH,
        )
        ...
        open_mdx_model_dir_Button = ttk.Button(
            mdx_net_frame,
            text=OPEN_MODELS_FOLDER_TEXT,
            command=lambda: OPEN_FILE_func(MDX_MODELS_DIR),
            width=VR_BUT_WIDTH,
        )
        ...
        mdx_batch_size_Option = ComboBoxEditableMenu(
            ...
            values=BATCH_SIZE,
            width=MENU_COMBOBOX_WIDTH,
            textvariable=self.mdx_batch_size_var,
            pattern=REG_BATCHES,
            default=BATCH_SIZE,
        )  #
        ...
        is_mdx_c_seg_def_Option = ttk.Checkbutton(
            mdx_net23_frame,
            text=SEGMENT_DEFAULT_TEXT,
            width=MDX_CHECKBOXS_WIDTH,
            variable=self.is_mdx_c_seg_def_var,
        )
        ...
        is_mdx_combine_stems_Option = ttk.Checkbutton(
            ...
            text=COMBINE_STEMS_TEXT,
            width=MDX_CHECKBOXS_WIDTH,
            variable=self.is_mdx23_combine_stems_var,
        )
        ...
        self.menu_placement(
            ...
            is_help_hints=True,
            close_function=self.menu_advanced_mdx_options_close_window,
            frame_list=frame_list,
        )
    ...
    def menu_advanced_ensemble_options(self):  # **
        ...
        self.menu_advanced_ensemble_options_close_window = lambda: (
            self.is_open_menu_advanced_ensemble_options.set(False),
        ...
        delete_entry_Option = ComboBoxMenu(
            ...
            textvariable=option_var,
            width=ENSEMBLE_CHECKBOXS_WIDTH + 2,
        )
        ...
        is_save_all_outputs_ensemble_Option = ttk.Checkbutton(
            ...
            variable=self.is_save_all_outputs_ensemble_var,
        )
        ...
        is_append_ensemble_name_Option = ttk.Checkbutton(
            ...
            width=ENSEMBLE_CHECKBOXS_WIDTH,
            variable=self.is_append_ensemble_name_var,
        )
        ...
        ensemble_close_Button = ttk.Button(
            custom_ens_opt_frame,
            text=CLOSE_WINDOW,
            command=lambda: self.menu_advanced_ensemble_options_close_window(),
        )
    ...
    def menu_advanced_align_options(self):  # **
        ...
        self.menu_advanced_align_options_close_window = lambda: (
            self.is_open_menu_advanced_align_options.set(False),
        ...
        phase_option_Option = ComboBoxMenu(
            ...
            textvariable=self.phase_option_var,
        ...
        phase_shifts_Option = ComboBoxMenu(
            advanced_align_opt_frame,
            textvariable=self.phase_shifts_var,
        ...
        is_save_align_Option = ttk.Checkbutton(
            ...
            width=MDX_CHECKBOXS_WIDTH,
            variable=self.is_save_align_var,
        )
        ...
        is_match_silence_Option = ttk.Checkbutton(
            ...
            width=MDX_CHECKBOXS_WIDTH,
            variable=self.is_match_silence_var,
        )
        ...
        is_spec_match_Option = ttk.Checkbutton(
            ...
            width=MDX_CHECKBOXS_WIDTH,
            variable=self.is_spec_match_var,
        )
        ...
        ensemble_close_Button = ttk.Button(
            advanced_align_opt_frame,
            text=CLOSE_WINDOW,
            command=lambda: self.menu_advanced_align_options_close_window(),
        )
        ensemble_close_Button.grid(pady=MENU_PADDING_1)
        ...
        self.menu_placement(
            ...
            is_help_hints=True,
            close_function=self.menu_advanced_align_options_close_window,
        )
    ...
    def menu_help(self):  # **
        ...
        self.menu_help_close_window = lambda: (
            self.is_open_menu_help.set(False),
    ...
    def menu_error_log(self):  #
        ...
        self.menu_error_log_close_window = lambda: (
            self.is_open_menu_error_log.set(False),
        ...
        copy_text_Label = tk.Label(
            ...
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_0}"),
            justify="center",
            fg="#f4f4f4",
        )
        ...
        error_log_return_Button = ttk.Button(
            error_log_frame,
            text=BACK_TO_MAIN_MENU,
            command=lambda: (self.menu_error_log_close_window(), self.menu_settings()),
        )
        ...
        error_log_close_Button = ttk.Button(
            error_log_frame,
            text=CLOSE_WINDOW,
            command=lambda: self.menu_error_log_close_window(),
        )
    ...
    def menu_preproc_model(self, tab):
        ...
        disable_pre_proc_model = lambda: (
            ...
            self.is_demucs_pre_proc_model_inst_mix_var.set(False),
        )
        pre_proc_model_toggle = (
            lambda: enable_pre_proc_model()
            if self.is_demucs_pre_proc_model_activate_var.get()
            else disable_pre_proc_model()
        )
        ...
        is_demucs_pre_proc_model_activate_Option = ttk.Checkbutton(
            ...
            width=DEMUCS_PRE_CHECKBOXS_WIDTH,
            variable=self.is_demucs_pre_proc_model_activate_var,
    ...
    def pop_up_save_current_settings(self):
        ...
        settings_save_name_Entry = ttk.Entry(
            settings_save_Frame,
            textvariable=settings_save_var,
            justify="center",
            width=25,
        )
        ...
        settings_save_Button = ttk.Button(
            settings_save_Frame,
            text=SAVE_TEXT,
            command=lambda: save_func() if validation(settings_save_var.get()) else invalid_message(),
        )
        ...
        stop_process_Button = ttk.Button(
            settings_save_Frame,
            text=CANCEL_TEXT,
            command=lambda: settings_save.destroy(),
        )
    ...
    def pop_up_user_code_input(self):
        ...
        user_code_confrim_Button = ttk.Button(
            user_code_Frame,
            text=CONFIRM_TEXT,
            command=lambda: self.download_validate_code(confirm=True, code_message=invalid_message_),
        )
        ...
        support_sub_Label = tk.Label(
            user_code_Frame,
            text=GET_DL_VIP_CODE_TEXT,
            font=(MAIN_FONT_NAME, f"{FONT_SIZE_1}"),
            foreground=FG_COLOR,
        )
        support_sub_Label.grid(pady=MENU_PADDING_1)

        uvr_patreon_Button = ttk.Button(
            user_code_Frame,
            text=UVR_PATREON_LINK_TEXT,
    ...
    def pop_up_change_model_defaults(self, top_window):
        ...
        def delete_entry():
            model_data = self.assemble_model_data(
        ...
        cancel_Button = ttk.Button(
            change_model_defaults_Frame,
            text=CANCEL_TEXT,
            command=lambda: change_model_defaults.destroy(),
        )
    ...
    def pop_up_set_vocal_splitter(self, top_window):
        ...
        try:
            ...
            disable_voc_split_model = lambda: (
                ...
                self.is_save_inst_set_vocal_splitter_var.set(False),
            )
            voc_split_model_toggle = (
                lambda: enable_voc_split_model()
                if self.is_set_vocal_splitter_var.get()
            ...
            deverb_opt_toggle = (
                lambda: enable_deverb_opt() if self.is_deverb_vocals_var.get() else disable_deverb_opt()
            )
            ...
            model_select_Option = ComboBoxMenu(
                ...
                textvariable=self.set_vocal_splitter_var,
                values=model_list,
                offset=310,
                width=READ_ONLY_COMBO_WIDTH,
            )
            ...
            save_inst_Button = ttk.Checkbutton(
                ...
                variable=self.is_save_inst_set_vocal_splitter_var,
            ...
            change_params_Button = ttk.Checkbutton(
                ...
                variable=self.is_set_vocal_splitter_var,
                width=SET_VOC_SPLIT_CHECK_WIDTH,
                command=voc_split_model_toggle,
            )
            ...
            deverb_vocals_Option = ComboBoxMenu(
                ...
                textvariable=self.deverb_vocal_opt_var,
            ...
            is_deverb_vocals_Option = ttk.Checkbutton(
                ...
                variable=self.is_deverb_vocals_var,
                command=deverb_opt_toggle,
            )
            ...
            cancel_Button = ttk.Button(
                set_vocal_splitter_Frame,
                text=CLOSE_WINDOW,
                command=lambda: set_vocal_splitter.destroy(),
                width=16,
            )
    ...
    def pop_up_mdx_model(self, mdx_model_hash, model_path):
        ...
        if is_compatible_model:
            ...
            set_stem_name_Option["menu"].add_radiobutton(
                label=INPUT_STEM_NAME,
                command=tk._setit(
                    primary_stem_var,
                    INPUT_STEM_NAME,
                    lambda e: self.pop_up_input_stem_name(primary_stem_var, mdx_model_set),
                ),
            )
            ...
            mdx_dim_t_set_Option = ComboBoxEditableMenu(
                mdx_model_set_Frame,
                values=("7", "8"),
                textvariable=mdx_dim_t_set_var,
                pattern=REG_SHIFTS,
                default=mdx_dim_t_set_var.get(),
                width=COMBO_WIDTH,
                is_stay_disabled=is_ckpt,
            )
            ...
            mdx_dim_f_set_Label = self.menu_sub_LABEL_SET(mdx_model_set_Frame, MDX_MENU_WAR_TEXT)
            mdx_dim_f_set_Label.grid(pady=MENU_PADDING_1)
            mdx_dim_f_set_Option = ComboBoxEditableMenu(
                mdx_model_set_Frame,
                values=(MDX_POP_DIMF),
                textvariable=mdx_dim_f_set_var,
                pattern=REG_SHIFTS,
                default=mdx_dim_f_set_var.get(),
                width=COMBO_WIDTH,
                is_stay_disabled=is_ckpt,
            )
            ...
            mdx_n_fft_scale_set_Option = ComboBoxEditableMenu(
                mdx_model_set_Frame,
                values=(MDX_POP_NFFT),
                textvariable=mdx_n_fft_scale_set_var,
                pattern=REG_SHIFTS,
                default=mdx_n_fft_scale_set_var.get(),
                width=COMBO_WIDTH,
                is_stay_disabled=is_ckpt,
            )
            ...
            mdx_compensate_Entry = ComboBoxEditableMenu(
                mdx_model_set_Frame,
                values=("1.035", "1.08"),
                textvariable=mdx_compensate_var,
                pattern=REG_VOL_COMP,
                default=mdx_compensate_var.get(),
                width=COMBO_WIDTH,
            )
    ...
    def pop_up_mdx_c_param(self, mdx_model_hash):
        ...
        get_mdx_c_params = lambda dir, ext: tuple(
            os.path.splitext(x)[0] for x in os.listdir(dir) if x.endswith(ext)
        )
    ...
    def pop_up_vr_param(self, vr_model_hash):
        ...
        get_vr_params = lambda dir, ext: tuple(
            os.path.splitext(x)[0] for x in os.listdir(dir) if x.endswith(ext)
        )
        ...
        vr_model_stem_Option["menu"].add_radiobutton(
            ...
            command=tk._setit(
                vr_model_stem_var,
                INPUT_STEM_NAME,
                lambda e: self.pop_up_input_stem_name(vr_model_stem_var, vr_param_menu),
            ),
        )
    ...
    def pop_up_save_ensemble(self):
        ...
        validation = lambda value: False if re.fullmatch(REG_SAVE_INPUT, value) is None else True
        save_func = lambda: (
            self.pop_up_save_ensemble_sub_json_dump(
                self.ensemble_listbox_get_all_selected_models(), ensemble_save_var.get()
            ),
        ...
        if len(self.ensemble_listbox_get_all_selected_models()) <= 1:
            ...
            stop_process_Button = ttk.Button(
                ensemble_save_Frame,
                text=OK_TEXT,
                command=lambda: ensemble_save.destroy(),
            )
            ...
            mdx_param_set_Button = ttk.Button(
                ensemble_save_Frame,
                text=SAVE_TEXT,
                command=lambda: save_func() if validation(ensemble_save_var.get()) else invalid_message(),
            )
            mdx_param_set_Button.grid(pady=MENU_PADDING_1)
    ...
    def deletion_list_fill(
        ...
        def refresh_menu(remove=None):
            selection_list = (
                self.last_found_ensembles if menu_name == "deleteensemble" else self.last_found_settings
            )
            main_var = (
                self.chosen_ensemble_var if menu_name == "deleteensemble" else self.save_current_settings_var
            )
        ...
        def online_check():
            ...
            try:
                ...
                if self.lastest_version == current_patch and not is_start_up:
                    ...
                    is_beta_version = (
                        True
                        if self.lastest_version == PREVIOUS_PATCH_WIN and BETA_VERSION in current_patch
                        else False
                    )
                    ...
                    if not is_start_up:
                        ...
                        if OPERATING_SYSTEM == "Windows":
                            ...
                            self.download_update_path_var.set(
                                os.path.join(
                                    BASE_PATH,
                                    f"{self.lastest_version}{application_extension}",
                                )
                            )
                ...
                is_update_params = (
                    self.is_auto_update_model_params
                    if is_start_up
                    else self.is_auto_update_model_params_var.get()
                )
            ...
            try:
                if is_update_app:
                    ...
                    if os.path.isfile(self.download_update_path_var.get()):
                        ...
                        wget.download(
                            self.download_update_link_var.get(),
                            self.download_update_path_var.get(),
                    ...
                    if self.select_download_var.get() == DEMUCS_ARCH_TYPE and is_demucs_newer:
                        for model_num, model_data in enumerate(self.download_demucs_newer_models, start=1):
                            self.download_progress_info_var.set(
                                "{} {}/{}...".format(
                                    DOWNLOADING_ITEM,
                                    model_num,
                                    len(self.download_demucs_newer_models),
                                )
                            )
                        ...
                        if os.path.isfile(self.download_save_path_var.get()):
                            ...
                            wget.download(
                                self.download_link_path_var.get(),
                                self.download_save_path_var.get(),
                                bar=download_progress_bar,
                            )
    ...
    def update_available_models(self):
        ...
        def fix_name(name, mapper: dict):
            return next(
                (new_name for old_name, new_name in mapper.items() if name in old_name),
                name,
            )
        ...
        new_demucs_models = self.get_files_from_dir(
            DEMUCS_MODELS_DIR, (CKPT, ".gz", ".th")
        ) + self.get_files_from_dir(DEMUCS_NEWER_REPO_DIR, YAML)
        ...
        def loop_directories(
            ...
            option_list = (
                [fix_name(file_name, name_mapper) for file_name in model_list] if name_mapper else model_list
            )
            sorted_options = natsort.natsorted(option_list)
            option_list_option_menu = (
                sorted_options + [OPT_SEPARATOR, DOWNLOAD_MORE] if self.is_online else sorted_options
            )
            ...
            if (
                self.is_root_defined_var.get()
                and model_type == MDX_ARCH_TYPE
                and self.chosen_process_method_var.get() == MDX_ARCH_TYPE
            ):
        ...
        if new_models_found != self.last_found_models or is_online != self.is_online:
            ...
            mdx_model_list = loop_directories(
                ...
                new_mdx_models,
                MDX_ARCH_TYPE,
                name_mapper=self.mdx_name_select_MAPPER,
            )
            demucs_model_list = loop_directories(
                self.demucs_model_Option,
                self.demucs_model_var,
                new_demucs_models,
                DEMUCS_ARCH_TYPE,
                name_mapper=self.demucs_name_select_MAPPER,
            )
            ...
            if not self.chosen_ensemble_var.get() == CHOOSE_ENSEMBLE_OPTION:
                ...
                if not self.ensemble_main_stem_var.get() == CHOOSE_STEM_PAIR:
                    self.selection_action_ensemble_stems(
                        self.ensemble_main_stem_var.get(),
        ...
        self.last_found_ensembles = self.update_menus(
            option_widget=self.chosen_ensemble_Option,
        ...
        self.last_found_settings = self.update_menus(
            option_widget=self.save_current_settings_Option,
    ...
    def update_main_widget_states(self):
        ...
        def general_shared_buttons():
            place_widgets(self.is_gpu_conversion_Option_place, self.model_sample_mode_Option_place)

        def stem_save_options():
            place_widgets(
                self.is_primary_stem_only_Option_place,
                self.is_secondary_stem_only_Option_place,
            )

        def stem_save_demucs_options():
            place_widgets(
                self.is_primary_stem_only_Demucs_Option_place,
                self.is_secondary_stem_only_Demucs_Option_place,
            )
        ...
        if process_method == MDX_ARCH_TYPE:
            ...
            place_widgets(self.chosen_audio_tool_Label_place, self.chosen_audio_tool_Option_place)
            ...
            audio_tool_options = {
                ...
                TIME_STRETCH: [
                    lambda: self.model_sample_mode_Option_place(rely=5),
    ...
    def update_button_states(self):
        ...
        if self.chosen_process_method_var.get() == DEMUCS_ARCH_TYPE:
            ...
            if not self.demucs_model_var.get() == CHOOSE_MODEL:
                ...
                self.demucs_stems_Option.command(
                    lambda e: self.update_stem_checkbox_labels(self.demucs_stems_var.get(), demucs=True)
                )
    ...
    def update_stem_checkbox_labels(
        self, selection, demucs=False, disable_boxes=False, is_disable_demucs_boxes=True
    ):
    ...
    def process_preliminary_checks(self):
        ...
        if self.chosen_process_method_var.get() == ENSEMBLE_MODE:
            continue_process = (
                lambda: False if len(self.ensemble_listbox_get_all_selected_models()) <= 1 else True
            )
    ...
    def process_storage_check(self):
        ...
        space_details = (
            f"Detected Total Space: {int(total / 1.074e9)} GB's\n"
            + f"Detected Used Space: {int(used / 1.074e9)} GB's\n"
            + f"Detected Free Space: {int(free / 1.074e9)} GB's\n"
        )

        appropriate_storage = True

        if int(free / 1.074e9) <= int(2):
            self.error_dialoge([STORAGE_ERROR[0], f"{STORAGE_ERROR[1]}{space_details}"])
            appropriate_storage = False

        if int(free / 1.074e9) in [3, 4, 5, 6, 7, 8]:
            appropriate_storage = self.message_box(
                [
                    STORAGE_WARNING[0],
                    f"{STORAGE_WARNING[1]}{space_details}{CONFIRM_WARNING}",
                ]
            )
    ...
    def process_initialize(self):
        ...
        ) and not (self.inputPaths and os.path.isfile(self.inputPaths[0])):
    ...
    def process_get_baseText(self, total_files, file_num, is_dual=False):
        ...
        text = "{init_text} {file_num}/{total_files} ".format(
            init_text=init_text, file_num=file_num, total_files=total_files
        )
    ...
    def process_end(self, error=None):
        ...
        if error:
            ...
            confirm = messagebox.askyesno(parent=root, title=ERROR_OCCURED[0], message=error_message_box_text)
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                if not self.verify_audio(audio_file):
                    error_text_console = (
                        f'{self.base_text}"{os.path.basename(audio_file)}" {MISSING_MESS_TEXT}\n'
                    )
                    if total_files >= 2:
                ...
                if audio_tool_action not in [
                    ...
                    text_write = (
                        ("File 1", "File 2") if audio_tool_action == ALIGN_INPUTS else ("Target", "Reference")
                    )
    ...
    def process_determine_demucs_pre_proc_model(self, primary_stem=None):
        ...
        if (
            self.demucs_pre_proc_model_var.get() != NO_MODEL
            and self.is_demucs_pre_proc_model_activate_var.get()
        ):
            pre_proc_model = ModelData(
                self.demucs_pre_proc_model_var.get(),
    ...
    def check_only_selection_stem(self, checktype):
        ...
        stem_primary_label = (
            self.is_primary_stem_only_Demucs_Text_var.get()
            if is_demucs
            else self.is_primary_stem_only_Text_var.get()
        )
        stem_primary_bool = (
            self.is_primary_stem_only_Demucs_var.get() if is_demucs else self.is_primary_stem_only_var.get()
        )
        stem_secondary_label = (
            self.is_secondary_stem_only_Demucs_Text_var.get()
            if is_demucs
            else self.is_secondary_stem_only_Text_var.get()
        )
        stem_secondary_bool = (
            self.is_secondary_stem_only_Demucs_var.get()
            if is_demucs
            else self.is_secondary_stem_only_var.get()
        )

        if checktype == VOCAL_STEM_ONLY:
            return not (
                (not VOCAL_STEM_ONLY == stem_primary_label and stem_primary_bool)
                or (not VOCAL_STEM_ONLY in stem_secondary_label and stem_secondary_bool)
            )
        elif checktype == INST_STEM_ONLY:
            return (
                INST_STEM_ONLY == stem_primary_label
                and stem_primary_bool
                and self.is_save_inst_set_vocal_splitter_var.get()
            ...
            return (VOCAL_STEM_ONLY == stem_primary_label and stem_primary_bool) or (
                VOCAL_STEM_ONLY == stem_secondary_label and stem_secondary_bool
            )
        elif checktype == IS_SAVE_INST_ONLY:
            return (INST_STEM_ONLY == stem_primary_label and stem_primary_bool) or (
                INST_STEM_ONLY == stem_secondary_label and stem_secondary_bool
            )

    def determine_voc_split(self, models):
        is_vocal_active = self.check_only_selection_stem(VOCAL_STEM_ONLY) or self.check_only_selection_stem(
            INST_STEM_ONLY
        )

        if (
            self.set_vocal_splitter_var.get() != NO_MODEL
            and self.is_set_vocal_splitter_var.get()
            and is_vocal_active
        ):
        ...
        try:
            ...
            for file_num, audio_file in enumerate(inputPaths, start=1):
                ...
                for current_model_num, current_model in enumerate(model, start=1):
                    ...
                    write_to_console = lambda progress_text, base_text=base_text: self.command_Text.write(
                    ...
                    audio_file_base = (
                        audio_file_base
                        if not self.is_testing_audio_var.get() or is_ensemble
                    ...
                    if not is_ensemble:
                        audio_file_base = (
                            audio_file_base
                            if not self.is_add_model_name_var.get()
                            else f"{audio_file_base}_{current_model.model_basename}"
                        )
                ...
                if is_ensemble:
                    ...
                    if self.ensemble_main_stem_var.get() in [
                        FOUR_STEM_ENSEMBLE,
                        MULTI_STEM_ENSEMBLE,
                    ]:
            ...
            self.error_log_var.set(
                "{}{}".format(
                    error_text(self.chosen_process_method_var.get(), e),
                    self.get_settings_list(),
                )
            )
            self.command_Text.write(f"\n\n{PROCESS_FAILED}")
            self.command_Text.write(time_elapsed())
    ...
    def load_to_default_confirm(self):
        ...
        if self.change_state_lambda is not None and (
            self.is_open_menu_advanced_vr_options.get()
            or self.is_open_menu_advanced_mdx_options.get()
            or self.is_open_menu_advanced_demucs_options.get()
        ):
    ...
    def load_saved_vars(self, data):
        ...
        self.is_use_opencl_var = tk.BooleanVar(
            value=False
        )  # True if is_opencl_only else data['is_use_opencl'])#
    ...
    def get_settings_list(self):
        ...
        settings_list = "\n".join(
            "".join(f"{key}: {value}") for key, value in settings_dict.items() if not key == "user_code"
        )
