...
class SeperateAttributes:
    def __init__(
        ...
        if vocal_stem_path:
            ...
            self.audio_file_base_voc_split = lambda stem, split: os.path.join(
                self.export_path,
        ...
        self.stem_path_init = os.path.join(
        ...
        if model_data.process_method == DEMUCS_ARCH_TYPE:
            ...
            self.device = (
                cpu
                if self.is_other_gpu and not self.demucs_version in [DEMUCS_V3, DEMUCS_V4]
                else self.device
            )
    ...
    def load_cached_sources(self):
        if self.is_secondary_model and not self.is_pre_proc_model:
            self.write_to_console(
                INFERENCE_STEP_2_SEC_CACHED_MODOEL(self.process_method, self.model_basename)
            )
        elif self.is_pre_proc_model:
            self.write_to_console(
                INFERENCE_STEP_2_PRE_CACHED_MODOEL(self.process_method, self.model_basename)
            )
        else:
    ...
    def write_audio(self, stem_path: str, stem_source, samplerate, stem_name=None):
        ...
        def save_voc_split_instrumental(stem_name, stem_source, is_inst_invert=False):
            ...
            inst_stem_source = spec_utils.combine_arrarys(
                [self.master_inst_source, stem_source], is_swap=True
            )
            save_with_message(inst_stem_path, inst_stem_name, inst_stem_source)

        def save_voc_split_vocal(stem_name, stem_source):
            voc_split_stem_name = (
                LEAD_VOCAL_STEM_LABEL if stem_name == LEAD_VOCAL_STEM else BV_VOCAL_STEM_LABEL
            )
        ...
        def save_with_message(stem_path, stem_name, stem_source):
            is_deverb = self.is_deverb_vocals and (
                self.deverb_vocal_opt == stem_name
                or (
                    self.deverb_vocal_opt == "ALL"
                    and (
                        stem_name == VOCAL_STEM
                        or stem_name == LEAD_VOCAL_STEM_LABEL
                        or stem_name == BV_VOCAL_STEM_LABEL
                    )
                )
            )
        ...
        is_bv_model_lead = (
            self.is_bv_model_rebalenced and self.is_vocal_split_model and stem_name == LEAD_VOCAL_STEM
        )
        is_bv_rebalance_lead = (
            self.is_bv_model_rebalenced and self.is_vocal_split_model and stem_name == BV_VOCAL_STEM
        )
        is_no_vocal_save = (
            self.is_inst_only_voc_splitter
            and (stem_name == VOCAL_STEM or stem_name == BV_VOCAL_STEM or stem_name == LEAD_VOCAL_STEM)
            or is_bv_model_lead
        )
        ...
        if is_bv_rebalance_lead:
            master_voc_source = spec_utils.match_array_shapes(
                self.master_vocal_source, stem_source, is_swap=True
            )
...
class SeperateMDX(SeperateAttributes):
    def seperate(self):
        ...
        if self.primary_model_name == self.model_basename and isinstance(self.primary_sources, tuple):
            ...
            if self.is_mdx_ckpt:
                model_params = torch.load(self.model_path, map_location=lambda storage, loc: storage)[
                    "hyper_parameters"
                ]
        ...
        mdx_net_cut = (
            True if self.primary_stem in MDX_NET_FREQ_CUT and self.is_match_frequency_pitch else False
        )
        ...
        if not self.is_primary_stem_only:
            ...
            if not isinstance(self.secondary_source, np.ndarray):
                ...
                self.secondary_source = (
                    spec_utils.invert_stem(raw_mix, source) if self.is_invert_spec else mix.T - source.T
                )
        ...
        if (
            ...
            if len(stem_list) == 1:
                ...
                source_primary = (
                    ...
                    else sources[self.mdxnet_stem_select]
                )
            if self.is_secondary_model_activated and self.secondary_model:
                self.secondary_source_primary, self.secondary_source_secondary = process_secondary_model(
                    self.secondary_model,
            ...
            if not self.is_primary_stem_only:
                secondary_stem_path = os.path.join(
                    self.export_path,
                    f"{self.audio_file_base}_({self.secondary_stem}).wav",
                )
                if not isinstance(self.secondary_source, np.ndarray):
            ...
            if self.is_denoise_model:
                if VOCAL_STEM in sources.keys() and INST_STEM in sources.keys():
                    sources[VOCAL_STEM] = vr_denoiser(
                        sources[VOCAL_STEM], self.device, model_path=self.DENOISER_MODEL
                    )
    ...
    def seperate(self):
        ...
        if (
            ...
            for stem_name, stem_value in self.demucs_source_map.items():
                    ...
                    if self.secondary_model_4_stem[stem_value]:
                        ...
                        if isinstance(stem_source_secondary, np.ndarray):
                            stem_source_secondary = stem_source_secondary[
                                1
                                if self.secondary_model_4_stem[stem_value].demucs_stem_count == 2
                                else stem_value
                            ].T
                ...
                stem_source = self.process_secondary_stem(
                    stem_source,
            ...
            if not self.is_primary_stem_only:
                ...
                def secondary_save(sec_stem_name, source, raw_mixture=None, is_inst_mixture=False):
                    ...
                    if not is_inst_mixture:
                        ...
                        self.secondary_source = self.process_secondary_stem(
...
class SeperateVR(SeperateAttributes):
    ...
    def inference_vr(self, X_spec, device, aggressiveness):
        def _execute(X_mag_pad, roi_size):
            ...
            total_iterations = (
                patches // self.batch_size if not self.is_tta else (patches // self.batch_size) * 2
            )
...
def loading_mix(X, mp):
    ...
    for d in range(bands_n, 0, -1):
        ...
        X_spec_s[d] = spec_utils.wave_to_spectrogram(
            X_wave[d], bp["hl"], bp["n_fft"], mp, band=d, is_v51_model=True
        )
