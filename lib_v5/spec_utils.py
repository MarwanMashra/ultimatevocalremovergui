    ...
    if "mirroring" == a:
        mirror = np.flip(
            np.abs(
                spec_m[
                    :,
                    mp.param["pre_filter_start"] - 10 - input_high_end.shape[1] : mp.param["pre_filter_start"]
                    - 10,
                    :,
                ]
            ),
...
def change_pitch_semitones(y, sr, semitone_shift):
    ...
    for y_channel in y:
        y_pitch_tuned.append(
            librosa.resample(y_channel, sr, sr * factor, res_type=wav_resolution_float_resampling)
        )
...
def align_audio(
    ...
    for s in wav2_aligned_sources:
        wav2_aligned = (
            match_mono_array_shapes(s, wav1) if is_mono else match_array_shapes(s, wav1, is_swap=True)
        )
...
def phase_shift_hilbert(signal, degree):
    ...
    return (
        np.cos(np.radians(degree)) * analytic_signal.real - np.sin(np.radians(degree)) * analytic_signal.imag
    )
